...
const conversations = [
  {
    ...
    lang: 'DE',
    source: 'Messenger', 
    // Zmienione na neutralne szarości
...
const CountryToggle = ({ flag, name, sub, active, onClick, locked = false }: { flag: string, name: string, sub?: string, active: boolean, onClick: () => void, locked?: boolean }) => (
    ...
    >
        <div className="flex items-center gap-4">
            <span className="text-xl grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100 transition-all">{flag}</span>
            <div>
...
const GrowthStep = ({ number, title, description, children, side = 'left', last = false }: { number: string, title: string, description: string, children: React.ReactNode, side?: 'left' | 'right', last?: boolean }) => {
    ...
    return (
        <div ref={domRef} className={`relative flex flex-col md:flex-row items-center justify-between gap-12 md:gap-32 transition-all duration-1000 ease-out transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <div className="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-zinc-200 z-10 items-center justify-center ring-4 ring-white">
            </div>
            {/* Linia tylko na desktopie - subtelniejsza */}
            <div className={`hidden md:block absolute top-1/2 -z-0 h-px bg-zinc-200 w-1/2 ${side === 'left' ? 'left-1/2' : 'right-1/2'}`}></div>
            ...
            <div className={`w-full md:w-1/2 flex flex-col justify-center relative z-10 order-1 ${side === 'left' ? 'md:items-end md:text-right md:pr-24 md:order-1' : 'md:items-start md:text-left md:pl-24 md:order-2'}`}>
                <div>
                    <div className="inline-block px-3 py-1 bg-zinc-100 text-zinc-500 rounded-full text-[10px] font-bold mb-4 tracking-widest border border-zinc-200/50 uppercase">Krok {number}</div>
...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white overflow-x-hidden font-sans text-zinc-900 selection:bg-zinc-900 selection:text-white">
      ...
      <section className="relative pt-32 md:pt-48 pb-16 md:pb-32 text-center overflow-hidden">
        ...
        <div className="px-4 md:px-6 max-w-5xl mx-auto relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16 px-4">
            ...
            <button className="group relative w-full sm:w-auto px-8 py-3.5 bg-white text-zinc-700 rounded-full border border-zinc-200 text-[15px] font-semibold hover:bg-zinc-50 hover:border-zinc-300 transition-all flex items-center justify-center gap-2 active:scale-95">
                <svg className="w-4 h-4 text-zinc-900" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                Zobacz demo
            </button>
          </div>
      ...
      <div className="w-full max-w-7xl mx-auto px-2 md:px-6 -mt-10 mb-20 md:mb-40 relative z-20">
        {/* Zmiana cieni na neutralne */}
        <div className="bg-white rounded-2xl md:rounded-[32px] border border-zinc-200 shadow-2xl shadow-zinc-200/50 overflow-hidden flex flex-col h-[80vh] md:h-[800px] ring-1 ring-zinc-900/5">
          {/* Mockup Header - styl macOS/modern */}
          ...
          <div className="flex flex-1 overflow-hidden relative bg-zinc-50/50">
            ...
            <div className={`${activeChatId ? 'hidden md:flex' : 'flex'} w-full md:w-[320px] border-r border-zinc-100 bg-white flex-col overflow-y-auto custom-scrollbar absolute md:relative z-10 h-full`}>
              <div className="p-4 md:p-5 flex justify-between items-center sticky top-0 bg-white z-10 border-b border-zinc-50">
                ...
                <div className="w-6 h-6 rounded-lg bg-zinc-50 flex items-center justify-center text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600 transition-colors cursor-pointer">
                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"/></svg>
                </div>
              </div>
              <div className="p-2 space-y-1">
                {conversations.map((chat) => (
                  <button key={chat.id} onClick={() => setActiveChatId(chat.id)} className={`w-full text-left p-3 rounded-xl border transition-all duration-200 group flex items-start gap-3 relative ${activeChatId === chat.id ? 'bg-zinc-50 border-zinc-200/60 shadow-sm z-10' : 'bg-white border-transparent hover:bg-zinc-50'}`}>
                    <div className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center text-sm font-bold ${activeChatId === chat.id ? 'bg-zinc-900 text-white' : 'bg-zinc-100 text-zinc-500'} transition-colors`}>{chat.name.charAt(0)}</div>
                    <div className="flex-1 min-w-0 py-0.5">
                        <div className="flex justify-between items-center mb-0.5">
                            <span className={`text-sm font-semibold truncate ${activeChatId === chat.id ? 'text-zinc-900' : 'text-zinc-700'}`}>{chat.name}</span>
            ...
            {/* Main Chat Area */}
            <div className={`${activeChatId ? 'flex' : 'hidden md:flex'} flex-1 flex-col bg-white relative w-full h-full`}>
              {/* Chat Header */}
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 bg-white">
                {activeChat.messages.map((msg) => (
                  <div key={msg.id} className={`flex gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300 ${msg.sender === 'agent' ? 'justify-end' : ''}`}>
                    ...
                    {msg.sender === 'user' && (
                        <div className="w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold bg-zinc-100 text-zinc-600 mt-2">{activeChat.name.charAt(0)}</div>
                    )}
                    ...
                    <div className={`flex flex-col max-w-[85%] md:max-w-[65%] ${msg.sender === 'agent' ? 'items-end' : 'items-start'}`}>
                      <div className={`p-4 md:p-5 rounded-2xl text-[15px] leading-relaxed shadow-sm relative group transition-all
                        ...
                        <div>{msg.text}</div>
                        {msg.translation && (
                          <div className={`mt-3 pt-2 text-xs border-t flex items-start gap-2 ${msg.sender === 'agent' ? 'border-zinc-700 text-zinc-400' : 'border-zinc-100 text-zinc-400'}`}>
                      ...
                      <span className="text-[10px] text-zinc-300 mt-2 px-1 font-medium">{msg.timestamp}</span>
                    </div>

                    {msg.sender === 'agent' && (
                        <div className="w-8 h-8 rounded-full bg-white border border-zinc-200 shrink-0 flex items-center justify-center text-zinc-900 text-xs font-bold mt-2 shadow-sm">Ty</div>
                    )}
                  </div>
                ))}

                {/* AI Suggestion Chip - Monochromatic */}
                {activeChat.messages.length > 0 && activeChat.messages[activeChat.messages.length-1].sender === 'user' && (
                    <div className="flex justify-center mt-6">
                        <button className="bg-white hover:bg-zinc-50 text-zinc-600 text-xs font-medium px-4 py-2 rounded-full flex items-center gap-2 transition-all border border-zinc-200 shadow-sm hover:shadow-md hover:border-zinc-300">
                            <div className="w-4 h-4 rounded-full bg-zinc-900 flex items-center justify-center text-white text-[8px] font-bold">AI</div>
                            <span>Sugestia: "Wyślemy nowy produkt jutro."</span>
                            <span className="text-zinc-300 text-[10px] ml-1">Enter ↵</span>
                        </button>
                    </div>
                )}
              </div>
              ...
              <div className="p-4 md:p-6 bg-white border-t border-zinc-100">
                <div className="flex gap-2 p-2 bg-zinc-50 border border-zinc-200 rounded-2xl focus-within:ring-2 focus-within:ring-zinc-900/5 focus-within:border-zinc-300 transition-all">
                  ...
                  <div className="flex items-center gap-1 pr-1">
                     ...
                     <button className="bg-zinc-900 hover:bg-black text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all shadow-md shadow-zinc-900/10 active:scale-95">Wyślij</button>
                  </div>
                </div>
                <div className="mt-3 flex items-center justify-center gap-2 text-[10px] text-zinc-400 font-medium">
                    <svg className="w-3 h-3 text-zinc-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
      ...
      <section id="how-it-works" className="py-24 md:py-40 px-4 md:px-6 max-w-7xl mx-auto w-full">
        ...
        <div className="relative">
            <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-px border-l border-dashed border-zinc-200 -translate-x-1/2"></div>
            ...
            <div className="space-y-24 md:space-y-40 relative">
                <GrowthStep number="1" title="Szybka instalacja" description="Instalujesz widget i panel w kilka minut. Twój obecny zespół supportu jest gotowy do działania." side="right">
                    <div className="bg-white p-8 rounded-[32px] border border-zinc-100 shadow-2xl shadow-zinc-200/40 w-full max-w-sm ml-auto transform hover:-translate-y-2 transition-transform duration-500 relative group">
                        <div className="flex items-center gap-5 mb-8">
                            <div className="w-14 h-14 rounded-2xl bg-zinc-50 border border-zinc-100 flex items-center justify-center text-zinc-900 shadow-sm"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg></div>
                            <div><div className="text-base font-bold text-zinc-900">System gotowy</div><div className="text-xs text-zinc-400 font-medium mt-1">Status: <span className="text-emerald-600">Aktywny</span></div></div>
                        </div>
                        <div className="h-1.5 w-full bg-zinc-100 rounded-full overflow-hidden"><div className="h-full bg-zinc-900 w-[85%] rounded-full"></div></div>
                    </div>
                </GrowthStep>
                <GrowthStep number="2" title="Wybór rynków" description="W panelu zaznaczasz kraje, na które chcesz wejść. Tłumaczenie AI włącza się automatycznie." side="left">
                    <div className="bg-white p-8 rounded-[32px] border border-zinc-100 shadow-2xl shadow-zinc-200/40 w-full max-w-sm mr-auto transform hover:-translate-y-2 transition-transform duration-500">
                        <div className="flex flex-wrap gap-2 mb-8">
                            ...
                            <span className="px-4 py-2 bg-zinc-900 rounded-lg text-sm font-semibold text-white shadow-lg shadow-zinc-900/20">Niemcy 🇩🇪</span>
                            <span className="px-4 py-2 bg-zinc-50 rounded-lg text-sm font-semibold text-zinc-600 border border-zinc-100">Francja 🇫🇷</span>
                        </div>
                ...
                <GrowthStep number="4" title="Wzrost przychodów" description="Klienci kupują chętniej, gdy mogą porozmawiać w swoim języku. Ty oszczędzasz na zespole, a słupki sprzedaży rosną." side="left" last={true}>
                    <div className="bg-white p-8 rounded-[32px] border border-emerald-100 shadow-2xl shadow-emerald-500/5 w-full max-w-sm mr-auto relative overflow-hidden transform hover:-translate-y-2 transition-transform duration-500 group">
                        <div className="absolute top-0 right-0 w-40 h-40 bg-emerald-50 rounded-full blur-3xl -mr-10 -mt-10 opacity-60 group-hover:opacity-100 transition-opacity"></div>
