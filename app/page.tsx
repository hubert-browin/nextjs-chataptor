...
const MarketCounter = () => {
    ...
    return (
        <div className="w-full space-y-4">
            <div className="bg-slate-900 rounded-2xl p-4 text-center shadow-lg relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/20 to-purple-500/20"></div>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1 relative z-10">Twój potencjalny zasięg</p>
                <div className="text-3xl md:text-4xl font-extrabold text-white relative z-10 tabular-nums transition-all duration-300">
                    {formattedCount} <span className="text-sm text-slate-500 font-normal">klientów</span>
                </div>
            </div>
...
const CountryToggle = ({ flag, name, sub, active, onClick, locked = false }: { flag: string, name: string, sub?: string, active: boolean, onClick: () => void, locked?: boolean }) => (
    <div 
        ...
        className={`flex items-center justify-between p-3 rounded-xl border transition-all cursor-pointer ${active ? 'bg-white border-indigo-100 shadow-md' : 'bg-slate-50 border-slate-100 hover:bg-white'} ${locked ? 'opacity-70 cursor-default' : ''}`}
    >
        ...
        <div className={`w-10 h-6 rounded-full relative transition-colors duration-300 ${active ? 'bg-indigo-600' : 'bg-slate-300'}`}>
            <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-300 ${active ? 'left-[22px]' : 'left-1'}`}></div>
        </div>
    </div>
);
...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-white overflow-x-hidden font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900">
              ...
              <div className="hidden md:flex h-16 border-b border-slate-100 bg-white/80 backdrop-blur-sm items-center justify-between px-6 shrink-0 sticky top-0 z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-slate-500 flex items-center gap-2"><span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Online</span><span className="text-slate-300">•</span><span>Język: <strong className="uppercase">{activeChat.lang}</strong></span><span className="text-slate-300">•</span><span className="capitalize">{activeChat.source}</span></div>
                  </div>
                </div>
            ...
            <div className="space-y-24 md:space-y-32 relative">
                ...
                <GrowthStep number="2" title="Wybór rynków" description="W panelu zaznaczasz kraje, na które chcesz wejść. Tłumaczenie AI włącza się automatycznie." side="left">
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 w-full max-w-sm mr-auto transform hover:-translate-y-1 transition-transform duration-300">
                        ...
                        <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-700">Dostępność</span><span className="text-xs text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md font-bold">Globalna</span></div>
                    </div>
                </GrowthStep>
      ...
      {/* --- FEATURE BENTO GRID (REDESIGNED V2) --- */}
      <section id="product" className="py-20 md:py-28 px-4 md:px-6 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 auto-rows-min">
          ...
          {/* Card 1 - Translation (Chat Style) */}
          <div className="group bg-white border border-slate-200 rounded-[2.5rem] p-8 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 md:col-span-2 flex flex-col md:flex-row gap-10 items-start md:items-center relative overflow-hidden">
            <div className="flex-1 relative z-10 text-left">
              ...
              <p className="text-slate-500 text-base leading-relaxed mb-6">
                 Brak opóźnień. Brak sztucznych barier. Klient pisze po niemiecku, Ty widzisz to od razu po polsku. Odpisujesz po polsku, klient dostaje wiadomość po niemiecku.
              </p>
              <div className="flex gap-2">
                  <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">Zero-latency</span>
                  <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">Bi-directional</span>
              </div>
            </div>
            
            <div className="relative w-full md:w-1/2 flex flex-col gap-4 bg-slate-50/50 p-5 rounded-3xl border border-slate-100">
                {/* Simulated Chat */}
                <div className="flex items-end gap-3">
                    ...
                    <div className="bg-white p-3 rounded-2xl rounded-bl-none shadow-sm border border-slate-100 text-sm text-slate-700">
                        Wo ist mein Paket?
                        <div className="mt-2 pt-2 border-t border-slate-100 text-xs text-indigo-600 font-medium flex items-center gap-1">
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 14l-7 7m0 0l-7-7"></path></svg>
                            Gdzie jest moja paczka?
                        </div>
                    </div>
                </div>
                <div className="flex items-end gap-3 justify-end mt-2">
                    <div className="bg-indigo-600 p-3 rounded-2xl rounded-br-none shadow-md text-sm text-white text-right">
                        Sprawdzam to...
                        <div className="mt-2 pt-2 border-t border-white/20 text-xs text-indigo-200 font-medium flex items-center gap-1 justify-end">
                            Ich überprüfe das...
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 10l7-7m0 0l7 7"></path></svg>
                        </div>
                    </div>
                    <div className="w-8 h-8 rounded-full bg-slate-900 flex items-center justify-center text-xs text-white">PL</div>
                </div>
            </div>
          </div>

          {/* Card 2 - Omnichannel (Tall) */}
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 md:row-span-2 flex flex-col relative overflow-hidden">
            ...
            <div className="relative z-10 flex-1 space-y-4">
                ...
                {[
                    ...
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all cursor-pointer group">
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${item.bg} ${item.color} shadow-sm group-hover:scale-110 transition-transform`}>
                                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">{item.icon}</svg>
                            </div>
                            <span className="font-bold text-slate-700">{item.name}</span>
                        </div>
                        {item.status === 'active' ? (
                            <div className="flex items-center gap-2 px-2 py-1 bg-white rounded-md border border-slate-100 shadow-sm">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Połączono</span>
                            </div>
                        ) : (
          ...
          {/* Card 3 - MARKET UNLOCK (Interactive) */}
          <div className="group bg-white border border-slate-200 rounded-[2.5rem] p-8 hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 relative overflow-hidden flex flex-col justify-between">
             <div className="relative z-10 mb-4">
                ...
                <p className="text-slate-500 text-sm">Nowe rynki jednym kliknięciem.</p>
            </div>
          ...
          <div className="group bg-white border border-slate-200 rounded-[2.5rem] p-8 hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 relative overflow-hidden flex flex-col">
             <div className="relative z-10 mb-6">
                ...
                <p className="text-slate-500 text-sm">Pop-upy, newslettery i ratowanie koszyków.</p>
            </div>
            
            {/* Mini Marketing UI */}
            <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 flex-1 flex flex-col justify-between">
                <div className="flex justify-between items-center mb-2">
                    <span className="text-[10px] font-bold text-slate-400 uppercase">Kampania: Black Friday</span>
                    <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                </div>
                <div className="flex gap-2 mb-3">
                    <div className="bg-white p-2 rounded-lg border border-slate-100 flex-1 text-center">
                        <div className="text-[10px] text-slate-400">Open Rate</div>
                        <div className="text-sm font-bold text-slate-900">42%</div>
                    </div>
                    <div className="bg-white p-2 rounded-lg border border-slate-100 flex-1 text-center">
                        <div className="text-[10px] text-slate-400">Kliknięcia</div>
                        <div className="text-sm font-bold text-indigo-600">1.2k</div>
                    </div>
                </div>
                {/* Pop-up Preview */}
                <div className="relative bg-white border border-slate-200 rounded-lg p-3 shadow-sm transform rotate-1 hover:rotate-0 transition-transform">
                    <div className="text-xs font-bold text-slate-900 mb-1">Czekaj! 🎁</div>
                    <div className="text-[10px] text-slate-500 mb-2">Mamy dla Ciebie -15% na pierwsze zamówienie.</div>
                    <button className="w-full bg-slate-900 text-white text-[10px] font-bold py-1 rounded">Odbierz kod</button>
                </div>
            </div>
          </div>
      ...
      <footer id="contact" className="py-16 md:py-20 bg-white border-t border-slate-200">
        <div className="container mx-auto px-6 max-w-7xl">
            ...
            <div className="space-y-6 md:space-y-8">
                ...
                <p className="text-slate-500 font-light leading-relaxed max-w-md text-base md:text-lg">Brama na świat dla Twojego e-commerce. Przełamujemy bariery językowe, łącząc AI z ludzką empatią.</p>
            </div>
            <div className="space-y-6 md:space-y-8">
            ...
            <div className="space-y-6 md:space-y-8">
                ...
                <li><a href="#" className="hover:text-slate-900 transition-colors flex items-center gap-2"><span className="w-1.5 h-1.5 bg-slate-300 rounded-full"></span>Polityka prywatności</a></li>
            ...
            <p className="text-slate-400 text-sm font-medium text-center">© {new Date().getFullYear()} Wszystkie prawa zastrzeżone.</p>
